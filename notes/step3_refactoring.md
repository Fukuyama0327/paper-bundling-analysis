...
- **進行中**: 全整数PWLでの**フルグリッド**（D=15〜50km、M=2〜6の36ケース、`data/processed/optimization_results_closed_form_20251207_200558.csv`が持つのと同じ組み合わせ）の再構築をWindows側で実行中。完了後、同ファイルを更新し、qのフル精度化（0-2章）を反映した最終版とする。詳細は次項参照。

## 5. フルグリッド実行で分かったこと・最終的な進め方（2026-07-11〜12）

### 5-1. 難ケースの実態とスクリプトの対応

D=15km,M=6のような「距離制約が厳しい割に地域数が多い」ケースは、全整数PWLだと探索空間が非常に大きく、証明（下界を縮める側）に長時間かかることが実際の実行で判明した（1回目試行: 21分でギャップ20%、2回目: 2.2時間でギャップ0.10%）。これを受けて`scripts/run_gurobi_districting.py`に2つの機能を追加した。

- **`--warm-start-min-m N`**: M≧Nのケースだけ、先に20点PWL近似で速く解いた結果をGurobiのMIPスタート（初期解）として与える。M<Nのケース（元々数秒で解ける）は事前処理を挟まないので余計な時間はかからない。20点PWL単体を採用しない理由（近似の「0%ギャップ」は真の閉形式目的関数に対しては無保証で、実際D=35km,M=3で20点PWLの解は厳密には劣っていた）は0-2章・7-1章参照。
- **`--mip-gap`**: 全整数PWLのまま、指定割合以内のギャップで打ち切る。PWL節点が全整数なのでPWL値=厳密値であり、このギャップは近似ではなく真の目的関数に対する保証付きの上限になる。

最終的な実行コマンド（`--warm-start-min-m 4 --mip-gap 0.005`）は`docs/multi_pc_git_python_notes.md` 7章に記載。

### 5-2. 実行基盤: `Start-Process`では不十分、タスクスケジューラに切り替え

VS Code Remote切断対策として`Start-Process`でデタッチ実行する方法（`docs/multi_pc_git_python_notes.md` 7-1章）を最初に試したが、**フルグリッド実行で2回とも、エラーなく途中でプロセスが消える**という事象が発生した（原因はスリープではなく特定できず）。Windowsのタスクスケジューラ（`LogonType S4U`、ログイン・リモート接続と無関係に実行）に切り替えて解決した。手順・ハマりどころ（アカウント指定は`whoami`の生出力を使う、リダイレクトは`cmd.exe /c`でラップする必要がある等）は`docs/multi_pc_git_python_notes.md` 7-2章に詳細を記載。

### 5-3. `run_gurobi_districting.py`の出力方針も統一

`make_all_figures.py`と同じ「正本＋アーカイブ」の二段構成を追加した（3b章参照）。`--output`/`--solutions-output`が正本（実行のたびに上書き）、`outputs/runs/<タイムスタンプ>/`が実行ごとのアーカイブ（デフォルトON、`--no-archive`で無効化、`run.meta.json`にgit HEADも記録）。

### 5-4. コードレビューで見つかった3件の実質的なバグ・設計問題（2026-07-12、修正済み）

1. **デフォルトパスの不一致**: `reevaluate_optimization_objectives.py`のデフォルト出力が`outputs/`、`plot_optimization_results.py`のデフォルト入力が`data/processed/`で食い違っていた（2026-07-11に図表側だけ`data/processed/`へ移した際の追従漏れ）。フルグリッド完了後にデフォルトのまま再評価すると図表が更新されない罠になっていた。前者のデフォルトを`data/processed/`に統一して解消。
2. **N=322を決める行政界チェックが省略可能オプションだった**: `step3_extract_rc_bridges.py`の`--boundary`はデフォルト`None`で、省略すると「無名橋2号」（座標が福島県側、0-1章参照）が残りN=323になる。デフォルトで行政界チェックを実行する（`config.SHAPEFILE_PATH`を使用）ように変更し、意図的に省く場合は`--no-boundary`を明示する設計にした。ファイルが無い場合はN=322の再現に必要である旨を示してエラー停止するようにした。
3. **距離行列pklとCSVの行順不一致（実際に発生していたバグ）**: `step3_compare_and_report.py`が、距離行列pklの`order`（座標順）と橋梁CSVの行順が異なりうることをチェックしていなかった。目的関数値自体は地域ごとの橋梁数だけで決まるため影響なかったが、管理者ベース行の`MaxDistanceWithin`（地域内最大距離）が誤った値になっていた。`order`とCSVの`shisetsu_id`を突き合わせ、集合が一致すれば並べ替え、一致しなければエラーにするよう修正。合成データで修正前後の挙動差を確認済み。
