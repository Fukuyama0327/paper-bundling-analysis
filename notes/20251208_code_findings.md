...
Windows GPU PCで `scripts/run_gurobi_districting.py` を実行し、代表ケースについて全整数PWLと20点PWLを比較した。

実行結果:

```text
data/processed/gurobi_validation_all_integer.csv
data/processed/gurobi_validation_20_nodes.csv
```

全整数PWL:

| D | M | Status | ObjectiveValue_Exact | RegionCounts | Time(s) |
|---:|---:|---:|---:|---|---:|
| 25 | 3 | 2 | 1.567601459468 | 277;35;10 | 2.901 |
| 35 | 3 | 2 | 1.261999978374 | 315;1;6 | 0.642 |
| 40 | 1 | 2 | 1.193288279181 | 322 | 0.032 |

20点PWL:

| D | M | Status | ObjectiveValue_Exact | RegionCounts | Time(s) |
|---:|---:|---:|---:|---|---:|
| 25 | 3 | 2 | 1.567601459468 | 10;35;277 | 1.851 |
| 35 | 3 | 2 | 1.262589495742 | 5;2;315 | 0.218 |
| 40 | 1 | 2 | 1.193288279181 | 322 | 0.058 |

読み取り:

- \(D=25\) km, \(M=3\) は地域順序が異なるだけで、地域別橋梁数は同じである。
- \(D=35\) km, \(M=3\) は20点PWLと全整数PWLで地域別橋梁数が異なり、厳密閉形式値もわずかに異なる。
- \(D=40\) km, \(M=1\) は完全に一致する。

結論:

- 20点PWLは概ね近いが、代表ケースで解が変わる場合がある。
- 査読論文化では、全整数PWLを主結果として採用する。
- \(N_m\) は整数であるため、全整数PWLは整数点上の閉形式 \(f(N,L)\) をそのまま表現していると扱える。
...
- その他の地域分割ケースも、可能であれば全整数PWLで再実行した結果を本文表に採用する。
