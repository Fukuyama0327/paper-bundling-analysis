...
一方，橋梁や道路を対象としたバンドリング（bundling）に関する研究も進展している．Qiao\cite{qiao2019}は，インディアナ州の道路・橋梁プロジェクトを対象に，バンドルが単価，工期，交通維持費，コスト超過リスク，入札者数などに与える影響を統計的に分析し，これらの複数要因を統合的に扱う多目的最適化モデルを構築した．さらに，Qiao et al.\cite{qiao2026}は，道路工事プロジェクト群を契約に割り当てる組合せ最適化問題として定式化し，貪欲ヒューリスティックにより既存の実務的バンドリング政策を上回る解を示した．同研究は，プロジェクトの組合せに応じて契約コストや入札者数が変化する内生性を明示的に扱う点で重要である．しかし，補修需要の確率的発生過程はモデル化されておらず，候補プロジェクトのリストは所与として与えられる．また，空間的な地域分割や，複数自治体をまたぐ連携範囲そのものは意思決定対象に含まれていない．

Assaf and Assaad\cite{assaf2023}は，実際のインフラプロジェクトを分析し，バンドリング戦略に影響を与える主要な意思決定要因として，地理的近接性，工種や規模の類似性，事業時期の整合性などを抽出・体系化した．さらに，Assaf et al.\cite{assaf2024}では，実データに基づきプロジェクト・バンドリングに内在する機会（効率化，コスト削減）と課題（調達の複雑化，リスクの集中）をモデル化し，要因間の連動パターンを明らかにすることで，より効果的な意思決定とリスク管理を支援する手法を提示している．また，米国連邦道路局（FHWA）は，Bridge Bundling Guidebook\cite{fhwa2019}を通じて，橋梁の効率的な更新・維持を目的に，複数橋の一括設計・施工・発注を行う制度設計，調達方式，リスクマネジメント，橋梁選定や適正規模設定などの実務手順を包括的に示している．
...
秋大会論文では，モンテカルロ・シミュレーションにより橋梁数 \(N\) と期待契約件数の対応関係を推定し，分数関数 \(f(N)=1.594N/(128.7+N)\) により近似していた．しかし，この近似式の係数はシミュレーション結果に対するフィッティング値であり，補修確率や同時発注上限との対応が直接的ではない．そこで本稿では，分数関数近似を主モデルとはせず，二項分布と天井関数から導かれる閉形式 \(f(N,L)\) を地域分割最適化の目的関数として用いる．
