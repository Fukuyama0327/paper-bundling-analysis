...
バンドリングの機会は，契約対象とし得る範囲内で，同じ期間に補修を必要とする橋梁がどの程度発生するかに依存する．橋梁が複数の管理主体に分散している場合，地理的に近接する補修需要であっても，既存の行政界が契約対象の組合せを制約し得る．米国Nebraska州では，個々の郡では橋梁バンドリングの便益を十分に得られない場合があることから，複数郡が行政界を越えて橋梁を一つの事業に集約する取組が実施されている（FHWA, 2019）．地方管理主体に分散した橋梁群について，どの範囲まで連携し，どの橋梁を同一の発注単位に含めるかは，契約バンドリングの課題の一つである．

一方，日本では，多くの橋梁が市区町村によって個別に管理されている．管理主体ごとに補修工事を発注する場合，同一契約に含め得る橋梁の範囲は各主体の管理区域に限定される．このため，管理橋梁数が少ない主体ほど，同一期間に発生する補修需要を一つの契約に集約する機会が限られる．また，共同発注等を行わない限り，地理的に近接する橋梁であっても，管理主体が異なれば別々の契約として扱われる．

この問題への対応として，複数の管理主体が行政界を越えて連携し，橋梁群を一体的に管理・発注することが考えられる．日本では国土交通省が「地域インフラ群再生戦略マネジメント（群マネ）」を推進し，市区町村の枠を超えた広域連携体制の構築を政策目標として掲げている（国土交通省, 2025）．
広域連携によって複数自治体の橋梁を一体的に管理・発注する場合，どのように管理エリアを設計すれば契約バンドリングによる契約集約効果を最大化できるかは，明らかになっていない．
...
| [2] | FHWA, n.d. | `fhwanebraska`（未登録） | FHWA: Case Study: Nebraska DOT County Bridge Match Program, Federal Highway Administration, U.S. Department of Transportation, n.d. https://www.fhwa.dot.gov/ipd/alternative_project_delivery/defined/bundled_facilities/case_study_ndot_county_bridge_match.aspx |
...
### [2] FHWA (n.d.) — Nebraska DOT County Bridge Match Program

- **本文で支える主張**: 米国でも地方公共機関が管理する橋梁が存在し，個々の郡ではバンドリングの便益を得にくい場合に，複数郡が行政界を越えて橋梁を束ねる取組が行われていること。
- **原文（Web page, Project Description）**:

> “Although each county itself may not have the means to reap the benefits of a bridge bundle, by working collaboratively together, counties can achieve this ability.”

- **判断**: 「小規模主体の管理スケール制約は世界各地で共通」と一般化する根拠にはならないが，「日本固有ではなく，国外にも同様の問題構造と越境連携の事例がある」という本文の主張を直接支持する。同ページは，複数郡が別々の郡にある橋梁を一事業へ束ねることを制度として認め，奨励していることも明記している。
- **原典**: https://www.fhwa.dot.gov/ipd/alternative_project_delivery/defined/bundled_facilities/case_study_ndot_county_bridge_match.aspx
- **書誌上の注意**: ページ上で公表年を確認できないため暫定的にn.d.とする。main.tex反映時に`fhwanebraska`の`\bibitem`を追加する。
