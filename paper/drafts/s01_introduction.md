...
> **方針メモ**
> - 国際誌向け。流れ：老朽化課題 → バンドリング → 分散管理の障壁 → 広域連携・地域分割 → ギャップ → 貢献
> - Introductionのサブセクションは最小限（または無し）。小タイトルは英語・短め。
...
> **残課題・確認事項**
> - "Background" の冒頭引用：老朽化インフラの維持管理費増大を示す一般的な国際文献が必要。→ [1] がまだ未決定。
...
> - サブセクションを切るかどうか（Introduction 一本にするか）は投稿先フォーマットに合わせて判断。
> - FDOT (2021) は現在 main.tex の参考文献リストに未収録。追加が必要。
...
|---|---|---|---|
| [1] | [引用]（老朽化インフラ） | **未決定** | 老朽化道路インフラの維持管理費増大を示す国際的な文献。要選定。 |
